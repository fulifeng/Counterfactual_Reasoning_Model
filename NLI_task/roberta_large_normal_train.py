...
parser.add_argument("--train_file", type =str, default= "./dataset/NLI/original/train.tsv")
parser.add_argument("--val_file", type =str, default= "./dataset/NLI/original/dev.tsv")
parser.add_argument("--test_file", type=str, default= "./dataset/NLI/original/test.tsv")
...
parser.add_argument("--save_folder", type=str, default="./NLI_tasks/roberta_large_normal_train/")
...
orig_test_max = 0
setup_seed(seed)
model = AutoModelForSequenceClassification.from_pretrained("roberta-large", num_labels = 3, from_tf = True).to(device)
...
for i in range(0, args.epochs): 
    ...
    with open( final_save_folder + "/acc_out","a+") as f:
        f.write("epoch:" + str(i) + " train_acc:" + str(acc1) + " orig_val_acc:" + str(acc2) + " orig_test_acc:" + str(acc3) + "\n")
    if i == args.epochs - 1:
        with open(args.save_folder + "/" + "final_acc", "a+") as f:
            f.write("random seed:" + str(seed) + "orig_val_acc: " + str(total_val_max) + "orig_test_acc: "+ str(orig_test_max) + "\n")
