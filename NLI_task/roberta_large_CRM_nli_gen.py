...
parser.add_argument("--save_folder", type=str, default ="./NLI_tasks/CRM_generator_result/")
...
parser.add_argument("--generator_path", type= str, default= "./NLI_task/roberta_large_CRM_generator.pt")
...
def calc_cf_sent_list(sent_list, model, tokenizer):
    model.eval()
    with torch.no_grad():
        real_out = model.roberta(**tokenizer(sent_list[:1], padding=True, truncation=True, max_length=512, return_tensors='pt').to(device)).last_hidden_state.detach()[:,:1,:]
        cf_out = torch.mean(model.roberta(**tokenizer(sent_list[1:5], padding=True, truncation=True, max_length=512, return_tensors='pt').to(device)).last_hidden_state.detach()[:,:1,:], dim = 0).view(1,1,-1)
...
def model_test_for_gen(batch_train_gen,classifier1, classifier2, cf_net):
    ...
    with torch.no_grad():
        for index in tqdm(range(len(batch_train_gen))):
            ...
            delta_out = classifier2(torch.cat(batch_train_gen[index][1]).view(len(label),1,-1)).view(len(label),1, -1)
            sample_out = classifier1(torch.cat(batch_train_gen[index][2]).view(len(label) * 2, 1, -1)).view(len(label),2, -1)
            output = cf_net(torch.cat([sample_out ,delta_out], dim=1).view(len(label),1,3, -1))
...
batch_test = shuffle_from_bs_1(batch_test_bs_1, 16)
...
total_out_folder = saving_folder
saving_folder = saving_folder + str(seed) + "/"
mk_dir(saving_folder)
...
for i in range(0, args.epochs): 
    ...
    plt.cla()
with open(total_out_folder + "/" + "final_out.log", "a+") as f:
    f.write("random seed:" + str(seed) + "test acc:" + str(final_test_acc) + "\n")