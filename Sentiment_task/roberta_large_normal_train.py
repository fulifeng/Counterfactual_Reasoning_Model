...
parser.add_argument("--save_folder", type=str, default="./roberta_large_normal_train/")
parser.add_argument("--log_name", type= str, default= "normal_train.log")
parser.add_argument("--plot_name", type = str, default= "normal_train.jpg")
...
batch_train = create_batch(orig_train_data, args.batchsize)
batch_val = create_batch(orig_val_data, args.batchsize)
batch_test = create_batch(orig_test_data, args.batchsize)

total_batch_val = create_batch(val_data, args.batchsize)
total_batch_test = create_batch(test_data, args.batchsize)
...
def model_test(batch_train, model):
    ...
    with torch.no_grad():
        for index in tqdm(range(len(batch_train))):
            ...
            encoder = tokenizer(batch_train[index][1], padding=True, truncation=True, max_length=256, return_tensors='pt' )
...
orig_test_max = 0
setup_seed(seed)
model = AutoModelForSequenceClassification.from_pretrained("roberta-large", num_labels = 2, from_tf = True).to(device)
...
for i in range(0, args.epochs): 
    ...
    acc4 = model_test(total_batch_val, model)
    acc5 = model_test(total_batch_test, model)
    # print(loss_total/len(batch_train))
    print(acc1, acc2, acc3 , acc4, acc5)
    mk_dir(args.save_folder)
    final_save_folder = args.save_folder + "/" + str(seed) + "/"
    mk_dir(final_save_folder)
    if acc4 > orig_val_max:
        orig_val_max = acc2
        orig_test_max = acc3
        torch.save(model, final_save_folder + "/roberta-large"  + ".pt")
    with open( final_save_folder + "/acc_out","a+") as f:
        f.write("epoch:" + str(i) + " train_acc:" + str(acc1) + " total_val_acc:" + str(acc4) + " orig_val_acc:" + str(acc2) + " total_test_acc:" + str(acc5) + " orig_test_acc:" + str(acc3) + "\n")
    if i == args.epochs - 1:
        with open(args.save_folder + "/" + "final_acc", "a+") as f:
            f.write("random seed:" + str(seed) + "orig_val_acc: " + str(orig_val_max) + "orig_test_acc: "+ str(orig_test_max) + "\n")
